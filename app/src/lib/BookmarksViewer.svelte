<script>
  ...
  async function bookmarkGame(gameName) {
      ...
      await fetch('/api/bookmarks/bookmarkGame', {
          method: 'POST',
          body: formData
      });
  ...
  async function removeBookmarkGame(gameName) {
      ...
      await fetch('/api/bookmarks/removeBookmarkedGame', {
          method: 'POST',
          body: formData
      });
...
{#if bookmarked_games.length > 0}
  ...
  {#each bookmarked_games as game}
      ...
      <div style="display: flex;">
          <button on:click={() => removeBookmarkGame(game.gameName)} id="btn_remove_bookmark"
...
{#if recommended_games.length > 0}
  ...
  {#each recommended_games as game}
      ...
      {/if}
      <div style="display: flex;">
          <button on:click={() => bookmarkGame(game.gameName)} id="btn_add_bookmark"
                  class="btn btn-primary" style="margin-right: 0.5rem;">
              Add Bookmark
          </button>
      </div>
